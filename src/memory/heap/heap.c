...
static uint32_t heap_align_value_to_upper(uint32_t val) {
    ...
    val -= (val % ORCAOS_HEAP_BLOCK_SIZE);
    val += ORCAOS_HEAP_BLOCK_SIZE;
...
static void* heap_block_to_address(struct heap* heap, int block) {
    return heap->saddr + (block * ORCAOS_HEAP_BLOCK_SIZE);
}
...
static int heap_address_to_block(struct heap* heap, void* address) {
    return ((int)(address - heap->saddr)) / ORCAOS_HEAP_BLOCK_SIZE;
}
...
static bool heap_validate_alignment(void* ptr) {
    return ((unsigned int)ptr % ORCAOS_HEAP_BLOCK_SIZE) == 0;
}
